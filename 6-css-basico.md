# CSS básico - Aprendendo desenvolvimento web | MDN
...
Ainda assim, precisamos aplicar o CSS ao seu documento HTML. Do contrário, o estilo CSS não irá afetar a maneira como o seu navegador mostra seu documento HTML (se você não estiver acompanhando o nosso projeto, leia [Lidando com arquivos](https://developer.mozilla.org/pt-BR/docs/Learn/Getting_started_with_the_web/Dealing_with_files) e [HTML básico](https://developer.mozilla.org/pt-BR/docs/Learn/Getting_started_with_the_web/HTML_basics) para descobrir o que você precisa fazer primeiro).

1. Abra seu arquivo `index.html` e cole o seguinte código em algum lugar no cabeçalho, ou seja, entre as tags [`<head>`](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/head) e `</head>`:
...
Seletor de elemento (às vezes, chamado tag ou seletor de tipo)
...
O elemento na página com o ID específicado. Em uma determinada página HTML, é uma boa prática usar um elemento por ID (e claro, um ID por elemento) mesmo que seja permitido usar o mesmo ID para vários elementos.

`#my-id` Seleciona `<p id="my-id">` ou `<a id="my-id">`

Seletor de classe

O(s) elemento(s) na página com a classe específicada (várias instâncias de classe podem aparecer em uma página).
...
Há muito mais seletores para explorar e você pode achar uma lista mais detalhada em nosso [Guia de seletores](https://developer.mozilla.org/en-US/docs/Learn/CSS/Building_blocks/Selectors).
...
1. Primeiro de tudo, volte e encontre a [fonte do Google Fonts](https://developer.mozilla.org/pt-BR/docs/Learn/Getting_started_with_the_web/What_will_your_website_look_like#fonte) que você armazenou em algum lugar seguro. Adicione o elemento [`<link>`](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/link) em algum lugar dentro do cabeçalho no `index.html` (novamente, em qualquer lugar entre as tags [`<head>`](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/head) e `</ head>`). Será algo parecido com isto:
...
3. Adicione as seguintes linhas em seu lugar, substituindo a linha do espaço reservado pela linha `font-family` que você obteve do Google Fonts. (`font-family` significa apenas a(s) fonte(s) que você deseja usar para o seu texto.) Esta regra primeiro define uma fonte base global e o tamanho da fonte para a página inteira (já que [`<html>`](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/html) é o elemento pai de toda a página, e todos os elementos dentro dele herdam o mesmo `font-size` e `font-family`):
...
4. Agora definiremos tamanhos de fonte para elementos que contêm texto dentro do corpo HTML ([`<h1>` (en-US)](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/Heading_Elements "Currently only available in English (US)"), [`<li>`](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/li) e [`<p>`](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/p)). Também centralizaremos o texto do nosso cabeçalho e definiremos a altura da linha e o espaçamento das letras no conteúdo do corpo para torná-lo um pouco mais legível:
...
Essa regra define uma cor de fundo para toda a página. Mude a cor acima para a cor [que você escolheu ao planejar seu site.](https://developer.mozilla.org/pt-BR/docs/Learn/Getting_started_with_the_web/What_will_your_website_look_like#cor_do_tema)
...
Agora para o elemento [`<body>`](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/body). Há algumas declarações aqui, então vamos passar por elas uma a uma:

- `width: 600px;` — Isso força o corpo a ter 600 pixels de largura.
- `margin: 0 auto;` — Quando você define dois valores em uma propriedade como `margin` ou `padding`, o primeiro valor afeta a parte superior do elemento **e** a parte inferior (tornando-os `0` nesse caso), e no segundo valor os lados esquerdo **e** direito (aqui, `auto` é um valor especial que divide o espaço horizontal uniformemente entre esquerda e direita). Você também pode utilizar um, três ou quatro valores, como documentado [aqui](https://developer.mozilla.org/pt-BR/docs/Web/CSS/margin#sintaxe).
- `background-color: #FF9500;` — como antes, isso define a cor de fundo do elemento. Usamos um tipo de laranja avermelhado no corpo, para opor ao azul escuro no elemento [`<html>`](https://developer.mozilla.org/pt-BR/docs/Web/HTML/Element/html), mas fique à vontade para ir em frente e experimentar.
- `padding: 0 20px 20px 20px;` — temos quatro valores definidos no padding, para criar um pouco de espaço em torno do nosso conteúdo. Dessa vez, estamos definindo sem padding na parte superior do corpo, e 20 pixels no lado esquerdo, na parte inferior e no lado direito. Os valores definem a parte superior, o lado direito, a parte inferior e o lado esquerdo, nessa ordem. Como com a `margin`, você também pode usar um, dois, ou três valores, conforme documentado na [sintaxe do padding](https://developer.mozilla.org/pt-BR/docs/Web/CSS/padding#sintaxe).
...
Você deve ter notado que há um espaço horrível no topo do corpo. Isso acontece porque os browsers aplicam algumas **estilizações padrão** ao [`<h1>` (en-US)](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/Heading_Elements "Currently only available in English (US)") (entre outros), mesmo quando você não aplicou nenhum CSS! Isso pode soar como uma má ideia, mas queremos ter uma legibilidade básica, mesmo em uma página sem estilos. Para nos livrarmos desse espaço, sobrescrevemos o estilo padrão, definindo `margin: 0;`.
...
Aqui, nós só arranhamos na superfície do CSS. Para descobrir mais, vá ao nosso [tópico de aprendizado CSS](https://developer.mozilla.org/pt-BR/docs/Learn/CSS).
